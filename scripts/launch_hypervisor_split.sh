...
_refresh_guest_files() {
...

    {
        ...
        for name in "${nodes[@]}"; do
            ...
            first=0
            if [[ "${env_prefix}" == "safety" ]]; then
                echo "SAFE_EDGE_OWN_IP=${guest_ip} SAFE_EDGE_CROSS_DOMAIN_IP=${NON_SAFETY_GUEST_IP} /system/bin/${name} > \"\${LOG_DIR}/${name}.log\" 2>&1 &"
            else
                echo "SAFE_EDGE_OWN_IP=${guest_ip} SAFE_EDGE_CROSS_DOMAIN_IP=${SAFETY_GUEST_IP} /system/bin/${name} > \"\${LOG_DIR}/${name}.log\" 2>&1 &"
            fi
