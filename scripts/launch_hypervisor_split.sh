...
# Detect the Linux→QEMU bridge subnet so guests can route back to the Linux host.
# virbr0 (libvirt NAT bridge) is the standard QEMU bridge on this machine.
_BRIDGE_SUBNET="$(ip -o -f inet addr show virbr0 2>/dev/null | awk '{print $4}' | head -1)"
: "${_BRIDGE_SUBNET:=192.168.122.0/24}"
_BRIDGE_HOST_IP="$(ip -o -f inet addr show virbr0 2>/dev/null | awk '{print $4}' | cut -d/ -f1 | head -1)"
: "${_BRIDGE_HOST_IP:=192.168.122.1}"
...
_refresh_guest_files() {
    ...
    local name bin cross_subnet gateway_ip
...

    {
        ...
        for name in "${nodes[@]}"; do
            ...
            first=0
            if [[ "${env_prefix}" == "safety" ]]; then
                local _safety_peers="${SAFETY_GUEST_IP}:8001,${SAFETY_GUEST_IP}:8002,${NON_SAFETY_GUEST_IP}:8011,${_BRIDGE_HOST_IP}:8020,${_BRIDGE_HOST_IP}:8030"
                echo "SAFE_EDGE_OWN_IP=${guest_ip} SAFE_EDGE_CROSS_DOMAIN_IP=${NON_SAFETY_GUEST_IP} SAFE_EDGE_HOST_IP=${_BRIDGE_HOST_IP} SAFE_EDGE_INITIAL_PEERS=${_safety_peers} /system/bin/${name} > \"\${LOG_DIR}/${name}.log\" 2>&1 &"
            else
                local _non_safety_peers="${NON_SAFETY_GUEST_IP}:8011,${NON_SAFETY_GUEST_IP}:8012,${SAFETY_GUEST_IP}:8001,${SAFETY_GUEST_IP}:8002,${_BRIDGE_HOST_IP}:8020,${_BRIDGE_HOST_IP}:8030"
                echo "SAFE_EDGE_OWN_IP=${guest_ip} SAFE_EDGE_CROSS_DOMAIN_IP=${SAFETY_GUEST_IP} SAFE_EDGE_HOST_IP=${_BRIDGE_HOST_IP} SAFE_EDGE_INITIAL_PEERS=${_non_safety_peers} /system/bin/${name} > \"\${LOG_DIR}/${name}.log\" 2>&1 &"
            fi
...
EOF
    ...
    seed_line+="; true"
    if [[ "${env_prefix}" == "safety" ]]; then
        cross_subnet="192.168.20.0/24"
        gateway_ip="${SAFETY_HOST_LINK_IP%/*}"
    else
        cross_subnet="192.168.10.0/24"
        gateway_ip="${NON_SAFETY_HOST_LINK_IP%/*}"
    fi
    cat > "${post_start}" <<EOF
# local/snippets/post_start.custom
# Generated by scripts/launch_hypervisor_split.sh
route add -net 224.0.0.0/4 vtnet0
route add -net ${cross_subnet} ${gateway_ip}
route add -net ${_BRIDGE_SUBNET} ${gateway_ip}
${seed_line}
mkdir -p /data/ssh
...
# Detect the Linux bridge IP (source IP used to reach the QNX hypervisor host).
# Docker containers with --network host will bind to this IP, so DDS nodes
# in the guests must use it as the peer IP for server (8020) and edge (8030).
LINUX_BRIDGE_IP="$(ip route get "${HOST_IP}" 2>/dev/null | awk '/src/{for(i=1;i<=NF;i++) if($i=="src") print $(i+1)}')"
if [[ -z "${LINUX_BRIDGE_IP}" ]]; then
    echo "Warning: could not detect Linux bridge IP for host→${HOST_IP}; defaulting to 127.0.0.1" >&2
    LINUX_BRIDGE_IP="127.0.0.1"
fi
echo "Linux bridge IP (SAFE_EDGE_HOST_IP): ${LINUX_BRIDGE_IP}"
